...
export function registerRecipeTools(server: McpServer, db: PrismaClient, userId: string) {
  server.registerTool(
    'list_recipes',
    {
      description: 'List recipes with optional tag filter. Returns id, title, tags, and hasContent flag (true if recipe has markdown content). Use get_recipe to read the full content.',
